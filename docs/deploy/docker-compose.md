...
The web UI is **baked into the Docker image** — no `web-svelte/dist` directory is needed on the host. If you deploy via Portainer or `docker compose pull`, it works out of the box.

The `docker-compose.override.yml` (used automatically for local `docker compose up`) mounts `./web-svelte/dist` over the image copy so local source changes appear without a rebuild. This override file is ignored when deploying remotely.
