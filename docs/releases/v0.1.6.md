...
- **Discover progress UI** — searching now shows animated source chips (MangaUpdates, AniList, MangaDex, NovelUpdates, WuxiaWorld, nhentai) instead of a generic skeleton. When results arrive, each chip turns green (or dims if that source returned nothing), staggered one by one.
