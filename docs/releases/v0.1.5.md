...
- **Migration bootstrap** — server no longer crashes on startup when upgrading from a pre-migration database (`table "external_sources" already exists`).
...
**Fix**: `MigrationBootstrap.Bootstrap()` now runs before `Migrate()`. It detects a pre-migration database (tables present, no history table) and inserts the appropriate migration records so `Migrate()` only applies genuinely new migrations. Idempotent — no effect on fresh or already-migrated databases.
