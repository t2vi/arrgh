// Full library flow for manhwa authority: discover → add → sync → chapters → download → view
// Source: AsuraScans (no CF protection, no CloakBrowser required)
...
describe(`library flow — asurascans manhwa (${QUERY})`, () => {
  it('1. discover returns a manhwa result', async () => {
    const results = await apiGet<DiscoverResult[]>(
      `/api/discover?q=${encodeURIComponent(QUERY)}`, token)

    // AsuraScans is a chapter source, not a discover authority — manhwa results surface via AniList.
    // MangaDex also indexes manhwa. We verify AsuraScans is queried via the sync log (step 5).
    const match = results.find(r => r.content_type === 'manhwa')
    expect(match, `no manhwa result for "${QUERY}"`).toBeDefined()
  })
  ...
  it('4. chapters loaded with sources', async () => {
    ...
    expect(chapters.length, `no chapters for "${addedTitle}" — asurascans sync may have failed`).toBeGreaterThan(0)
    expect(chapters.some(c => c.has_sources)).toBe(true)
  })

  it('5. sync log contains asurascans and sync-complete entries', async () => {
    expect(titleId).toBeTruthy()
    const logs = await apiGet<{ message: string }[]>(`/api/titles/${titleId}/sync-log`, token)
    const messages = logs.map(l => l.message)

    expect(messages.some(m => m.toLowerCase().includes('asurascans')),
      `asurascans not in sync log. Got: ${messages.join(' | ')}`).toBe(true)
