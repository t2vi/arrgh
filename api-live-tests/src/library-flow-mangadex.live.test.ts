// Full library flow for manga authority: discover → add → sync → chapters → download → view
// Source: MangaDex (no CF protection, no CloakBrowser required)
...
describe(`library flow — mangadex manga (${QUERY})`, () => {
  it('1. discover returns a manga result', async () => {
    const results = await apiGet<DiscoverResult[]>(
      `/api/discover?q=${encodeURIComponent(QUERY)}`, token)

    // MangaDex is a chapter source, not a discover authority — manga results surface via MangaUpdates.
    // We verify MangaDex is queried by checking the sync log after add (step 5).
    const match = results.find(r => r.content_type === 'manga')
    expect(match, `no manga result for "${QUERY}"`).toBeDefined()
  })
  ...
  it('4. chapters loaded with sources', async () => {
    ...
    expect(chapters.length, `no chapters for "${addedTitle}" — mangadex sync may have failed`).toBeGreaterThan(0)
    expect(chapters.some(c => c.has_sources)).toBe(true)
  })

  it('5. sync log contains mangadex and sync-complete entries', async () => {
    expect(titleId).toBeTruthy()
    const logs = await apiGet<{ message: string }[]>(`/api/titles/${titleId}/sync-log`, token)
    const messages = logs.map(l => l.message)

    expect(messages.some(m => m.toLowerCase().includes('mangadex')),
      `mangadex not in sync log. Got: ${messages.join(' | ')}`).toBe(true)
