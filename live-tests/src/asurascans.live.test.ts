...
import * as plugin from '../../plugins/asurascans/src/index'
import { connectBrowser, type BrowserConn } from './helpers'
import corpus from '../corpus/asurascans.json'
...
describe('asurascans — live snapshots', () => {
  for (const query of corpus.search) {
    it(`search: ${query}`, async ({ skip }) => {
      if (!conn) skip()

      plugin.init(conn!.makeContext('asurascans', 'search', query))
      ...
      plugin.init(conn!.makeContext('asurascans', 'chapters', query))
      const chapters = await plugin.chapters(first.id)
      expect(chapters).toMatchSnapshot()

      if (chapters.length === 0) return

      plugin.init(conn!.makeContext('asurascans', 'pages', query))
