...
export default function Library() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['manga', page, search],
    queryFn: () => api.listManga(page, search || undefined),
  ...
  const remove = useMutation({
    mutationFn: ({ id, deleteFiles }: { id: string; deleteFiles: boolean }) =>
      api.removeManga(id, deleteFiles),
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-5">
                {data.items.map((m) => (
                  <MangaCard
                    ...
                    onRemove={(deleteFiles) => remove.mutate({ id: m.id, deleteFiles })}
                    isRemoving={remove.isPending && remove.variables?.id === m.id}
                  />
                ))}
              </div>
