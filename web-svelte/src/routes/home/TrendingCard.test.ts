...
describe('TrendingCard', () => {
  it('renders skeleton pulse when cover_url is null', () => {
    const { container } = render(TrendingCard, { result: makeResult({ cover_url: null }), badge: 'HOT', onClick: () => {} })
  ...
  it('renders img when cover_url is set', () => {
    const { container } = render(TrendingCard, {
      result: makeResult({ cover_url: 'https://cdn.example.com/cover.jpg' }), badge: 'TOP', onClick: () => {},
    })
  ...
  it('renders /api/ cover_url directly without double-proxying', () => {
    const { container } = render(TrendingCard, {
      result: makeResult({ cover_url: '/api/media/meta-cover?key=berserk' }), badge: 'HOT', onClick: () => {},
    })
    const img = container.querySelector('img')
    expect(img).toBeTruthy()
  ...
  it('shows book emoji on img error not skeleton', async () => {
    const { container } = render(TrendingCard, {
      result: makeResult({ cover_url: 'https://cdn.example.com/broken.jpg' }), badge: 'HOT', onClick: () => {},
    })
    const img = container.querySelector('img')!
  ...
  it('shows title and author below cover', () => {
    render(TrendingCard, { result: makeResult(), badge: 'NEW', onClick: () => {} })
    expect(screen.getByText('Berserk')).toBeTruthy()
    expect(screen.getByText('Miura, Kentaro')).toBeTruthy()
  })

  it('shows badge', () => {
    render(TrendingCard, { result: makeResult(), badge: 'TOP', onClick: () => {} })
