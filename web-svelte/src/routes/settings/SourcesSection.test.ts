...
import type { SourceRow, PluginIndexEntry } from '../../lib/api'

function makeSource(overrides: Partial<SourceRow> = {}): SourceRow {
  return {
    id: 'src-1', name: 'MangaDex', base_url: 'http://localhost:4000/mangadex',
    enabled: true, has_api_key: false, is_community: false, content_types: ['manga'],
    ...overrides,
  }
}
...
function makePlugin(overrides: Partial<PluginIndexEntry> = {}): PluginIndexEntry {
  return {
    id: 'myplugin', name: 'My Plugin', version: '1.0.0', description: 'A test plugin',
    download_url: 'http://example.com/myplugin.js', bundled: false, default_explicit: false,
    content_types: ['manga'],
    ...overrides,
  }
}
...
beforeEach(() => {
  ...
  vi.mocked(api.addSource).mockResolvedValue(undefined)
...
describe('SourcesSection', () => {
  ...
  it('closes browse modal when X button clicked', async () => {
    ...
    const closeBtn = document.querySelector('.fixed button') as HTMLButtonElement
