...
use crate::{auth, db::Manga, AppState};
use super::ApiResult;
...
async fn add_manga(
    ...
    let manga_id: String = if let Some(eid) = existing_id {
        // Manga already in catalog — just refresh metadata that might be missing
        sqlx::query!(
            "UPDATE manga SET \
    ...
    // Add to requesting user's library (no-op if already subscribed)
