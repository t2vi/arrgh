...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/progress/title/{title_id}", get(list_title_progress))
...
async fn list_title_progress(
    ...
    let rows = sqlx::query_as!(
        ...
        r#"SELECT rp.id as "id!",
           ...
           WHERE c.title_id = ? AND rp.user_id = ?"#,
        title_id,
        user_id
    )
    .fetch_all(&state.db)
    ...
    let rows = sqlx::query_as!(
        ...
        r#"SELECT
               ...
               (SELECT c.id FROM chapters c
                ...
                WHERE c.title_id = m.id AND c.downloaded = 1
               ...
               (SELECT COUNT(*) FROM read_progress rp2
                ...
                WHERE c2.title_id = m.id AND rp2.completed = 1 AND rp2.user_id = ?) as "chapters_read!: i64",
               (SELECT COUNT(*) FROM chapters c3
                WHERE c3.title_id = m.id)         as "total_chapters!: i64"
           FROM titles m
           WHERE EXISTS (
               SELECT 1 FROM read_progress rp4
               JOIN chapters c4 ON c4.id = rp4.chapter_id
               WHERE c4.title_id = m.id AND rp4.completed = 1 AND rp4.user_id = ?
           )
           AND EXISTS (
               SELECT 1 FROM chapters c5
               LEFT JOIN read_progress rp5 ON rp5.chapter_id = c5.id AND rp5.user_id = ?
               WHERE c5.title_id = m.id AND c5.downloaded = 1
