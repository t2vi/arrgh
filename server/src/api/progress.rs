...
use crate::{db::ReadProgress, AppState};
use super::ApiResult;
...
async fn get_progress(
    ...
    let progress = sqlx::query_as!(
        ...
        r#"SELECT
           ...
           FROM read_progress WHERE chapter_id = ?"#,
        chapter_id
    )
    ...
    let rows = sqlx::query_as!(
        ...
        r#"SELECT
               ...
               (SELECT c.id FROM chapters c
                LEFT JOIN read_progress rp ON rp.chapter_id = c.id
               ...
               (SELECT COUNT(*) FROM read_progress rp2
                ...
                WHERE c2.manga_id = m.id AND rp2.completed = 1) as "chapters_read!: i64",
           ...
           WHERE EXISTS (
               ...
               WHERE c4.manga_id = m.id AND rp4.completed = 1
           )
           AND EXISTS (
               SELECT 1 FROM chapters c5
               LEFT JOIN read_progress rp5 ON rp5.chapter_id = c5.id
...
async fn update_progress(
    ...
    sqlx::query!(
        r#"INSERT INTO read_progress (id, chapter_id, current_page, completed, updated_at)
           VALUES (?, ?, ?, ?, ?)
           ON CONFLICT(chapter_id) DO UPDATE SET
             current_page = excluded.current_page,
             completed = excluded.completed,
             updated_at = excluded.updated_at"#,
        id, chapter_id, body.current_page, completed, now
    )
    .execute(&state.db)
    .await?;
